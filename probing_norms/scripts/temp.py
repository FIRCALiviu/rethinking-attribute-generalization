...
def get_supercategs_feature(feature,feature_to_concepts,concepts_to_id):
    ...
    idxs =  np.array( [*map (lambda x : concepts_to_id[x], feature_to_concepts[feature] )])
    rows = categs[idxs,:]
    
    club_counts = rows.sum(axis=0)

    non_empty = np.where(club_counts > 0)[0]
    counts_non_empty = club_counts[non_empty]

    k = int(np.ceil(len(non_empty) * 0.5))
    top_indices = non_empty[np.argsort(counts_non_empty)[-k:]]
    return categs,top_indices
...
def main():
    ...
    concepts_to_id = { v:i for i,v in enumerate(selected_concepts)}
    feature_neurons_allowed = dict()
    for feature in features_selected:
        categs,relevant_categs_idxs = get_supercategs_feature(feature, feature_to_concepts, concepts_to_id)
        ...
        with open(path,'rb') as f:
            ...
            categs = categs[:,relevant_categs_idxs]
            supercategs_activated_neuron = np.zeros((weights_selected.shape[-1],relevant_categs_idxs.shape[-1]),dtype=np.bool)
            for i in range(embeddings.shape[0]): # pentru toate conceptele
                activated = embeddings[i][weights_selected] > 0.3
