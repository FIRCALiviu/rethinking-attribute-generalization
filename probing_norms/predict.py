...
class File_result_manager_llm:
    file_Res = None

    def get_idxs_similar(cls):
        if File_result_manager_llm.file_Res is None:
            with open('./probing_norms/similar_idxs.txt') as f:
                numbers = [int(x) for x in f.read().split()]
                file_Res = numbers.copy()
                return numbers
        else:
            return file_Res.copy()

class File_result_manager_similarity:
    file_Res = None

    def get_idxs_similar():
        if File_result_manager_similarity.file_Res is None:
            with open('./temp_idxs.txt') as f:
                numbers = [int(x) for x in f.read().split()]
                file_Res = numbers.copy()
                return numbers
        else:
            return file_Res.copy()
def get_idxs_similar(method):
    match method:
        case 'similarity':
            return File_result_manager_similarity.get_idxs_similar()
        case "llm":
            return File_result_manager_similarity
...
def get_train_test_split_categs(
    ...
    *,
    ...
    for key in data:
        data[key] = [Split(tr_idxs=np.array(data[key]['train_idxs'],dtype=np.int32),te_idxs=np.array(data[key]['test_idxs']),metadata={'feature':key})]
    return data

def get_train_test_split_k_fold_excluded_similarity(
    labels,
    features,
    *,
    feature_to_concepts,
    ...
    def get_f(feature):
        ...
        tz = [exclude_from_test(Split(
                *idxss,
                metadata={"feature": feature},
            ),get_idxs_similar("similarity"))
            for idxss in rskf.split(binary_labels, binary_labels)
        ]
        tz.extend([exclude_from_test(Split(
                *idxss,
                metadata={"feature": feature},
            ),get_idxs_similar("similarity"))
            for idxss in rskf2.split(binary_labels, binary_labels)])
        return tz
    return {feature: get_f(feature) for feature in features}

def get_train_test_split_k_fold_excluded_llm(
    labels,
    features,
    *,
    feature_to_concepts,
...
GET_TRAIN_TEST_SPLIT = {
    ...
    "similarity-k-fold" : get_train_test_split_k_fold_excluded_similarity,
    "llm-k-fold" : get_train_test_split_k_fold_excluded_llm,
    "clustering" : get_train_test_split_k_means,
    "supercategs" : get_train_test_split_categs
}
...
def main(classifier_type, embeddings_level, feature_type, norms_type, split_type):
    ...
    FOLDER = "/root/output/{}-predictions/{}/{}".format(
        classifier_type, embeddings_level, "repeated-k-fold"
    )
