...
DIR_THINGS = Path("/root/data/things")
...
class LogisticRegression:
    def __init__(self,max_iter,class_weight,random_state,device='cuda',regularizition = 'l2'):
        super().__init__()
    ...
    def fit(self,x,y):
        ...
        x = torch.tensor(x,dtype=torch.float16).to(self.device) # check later if we need to split in batches
        y = torch.tensor(y).to(self.device)
        _ , vector_size = x.shape 
        torch.manual_seed(self.random_state)
        self.model = torch.nn.Linear(vector_size,2,dtype=torch.float16).to(self.device)

        max_iter = self.max_iter

        threshold = 10**-3
        criterion  = torch.nn.CrossEntropyLoss()
        optimizer = torch.optim.SGD(self.model.parameters(),lr = 10**-3)
        last_loss = 10**9

        for i in range(max_iter): # check later if we need to split in batches
            y_pred = self.model(x) 
            loss = criterion(y_pred,y) + self.get_regularization()
    ...
    def get_regularization(self):
        scale = 10**-4
        match self.regularizition :
            case None:
                return 0
            case "l2":
                return scale*(  torch.square(self.model.weight).sum() + torch.square(self.model.weight).sum() )
            case "l1":
                return scale*(  torch.abs(self.model.weight).sum() + torch.abs(self.model.weight).sum() )
