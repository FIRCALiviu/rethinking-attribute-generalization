...
class DINOV3(nn.Module):
    ...
    def forward(self, pixel_values):
        """
        Extract mean-pooled features from DINOv3.
        """
        # Guard again in case caller passed 5D/1-channel here directly
        ...
        last_hidden = outputs.last_hidden_state  # (batch, tokens, hidden_size)
        features = last_hidden.mean(dim=1)       # mean pool over tokens
        return features
...
def main(dataset_name, feature_type):
    ...
    batch_size = BATCH_SIZES.get(feature_type, 128)
    ...
    path_np = f"/root/output/features-image/{dataset_name}-{feature_type}.npz"
